...
var CMD_RE = new RegExp('([UDLR])([0-9]+)');
...
function parse(prog) {
    ...
    var cmds = prog.split(',');
    cmds.forEach(cmd => {
        var m = CMD_RE.exec(cmd)
        var dir = m[1];
        var dist = ~~m[2];
        console.log('cmd', cmd, m, dist, dir);
        draw(dir, dist);
    });
...
function run(prog1, prog2) {
    var st1 = parse(prog1);
    var st2 = parse(prog2);

    var collisions = [];
    for(var i=0; i<st1.length; i++) {
        if (i % 2000 === 0) console.log('i=', i);
        for(var j=0; j<st2.length; j++) {
            if (st1[i].c === st2[j].c) {
                console.log('collision', i, j, st1[i], st2[j]);
                collisions.push({i, j, a:st1[i], b:st2[j], d:st1[i].d, t: st1[i].t + st2[j].t});
            }
        }
    }

    collisions.sort((a, b) => {
        // return a.d - b.d; // star 1
        return a.t - b.t; // star 2
    })

    console.log('collisions', collisions);
    console.log('closest manhattan distance', collisions[0].d);
    console.log('closest total steps', collisions[0].a.t + collisions[0].b.t);
    console.log();
    console.log();
    console.log();
}

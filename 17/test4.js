...
Before R,6,L,8,R,8,R,6,L,8,R,8,R,4,R,6,R,6,R,4,R,4,L,8,R,6,L,10,L,10,R,4,R,6,R,6,R,4,R,4,L,8,R,6,L,10,L,10,R,4,R,6,R,6,R,4,R,4,L,8,R,6,L,10,L,10,R,6,L,8,R,8,L,8,R,6,L,10,L,10
Clean  {"pattern":"R,4,R,6,R,6,R,4,R,4","offset":12,"length":10,"dupes":2} {"pattern":"L,8,R,6,L,10,L,10","offset":22,"length":8,"dupes":3} {"pattern":"R,6,L,8,R,8","offset":0,"length":6,"dupes":2}
A: {"pattern":"R,4,R,6,R,6,R,4,R,4","offset":12,"length":10,"dupes":2}
B: {"pattern":"L,8,R,6,L,10,L,10","offset":22,"length":8,"dupes":3}
C: {"pattern":"R,6,L,8,R,8","offset":0,"length":6,"dupes":2}
After c,,,,,,c,,,,,,a,,,,,,,,,,b,,,,,,,,a,,,,,,,,,,b,,,,,,,,a,,,,,,,,,,b,,,,,,,,c,,,,,,b,,,,,,,
...
const FUN_A = 'R,4,R,6,R,6,R,4,R,4';
const FUN_B = 'L,8,R,6,L,10,L,10';
const FUN_C = 'R,6,L,8,R,8';
const MAIN = 'C,C,A,B,A,B,A,B,C,B';
...
runtime.onOutput = function(output) {
    if (printoutput || output > 150) {
        console.log('writing', output, cx, cy);
    }
    if (output == 10) {
        cy ++;
        if (cy > height) {
        ...
        canvas.set(cx, cy, String.fromCharCode(output));
        if (cx === 0) {
            lastline = '';
        }
        lastline += String.fromCharCode(output);
        cx ++;
        if (cx > width) {
            width = cx;
        }
...
for(var j=0; j<height; j++) {
    for(var i=0; i<width; i++) {
        var v0 = canvas.get(i, j);
        var v1 = canvas.get(i - 1, j);
        var v2 = canvas.get(i + 1, j);
        var v3 = canvas.get(i, j - 1);
        var v4 = canvas.get(i, j + 1);
        if (v0 == '#') {
            if (v1 == '#' && v2 == '#' && v3 == '#' && v4 == '#') {
                console.log('crossing at ', i, j);
                test1 += i * j;
            }
        }
    }
...
runUntilBlocked();
canvas.print();
console.log('Last prompt:', lastline);

console.log('Sending main');
runtime.addInput(MAIN + '\n');
...
runtime.addInput(FUN_A + '\n');
// sendlist(runtime, FUN_A);

runUntilBlocked();
console.log('Last prompt:', lastline);

console.log('Sending B');
runtime.addInput(FUN_B + '\n');
// sendlist(runtime, FUN_B);

runUntilBlocked();
console.log('Last prompt:', lastline);

console.log('Sending C');
runtime.addInput(FUN_C + '\n');
// sendlist(runtime, FUN_C);

...
class Runtime {
    ...
    addInput(input) {
        if (typeof(input) === 'number') {
            ...
            input.split('').forEach(c => {
                this.programinput.push(c.charCodeAt(0));
            });
        }

        if (this.verbose) {
            console.log(`${this.name}: Input queue is now: ${JSON.stringify(this.programinput)}`);
        }
    }
    ...
    step() {
        if (this.halted) {
            if (this.verbose) {
                console.log(`${this.name}: is halted`);
            }
            return false;
        }
        ...
        if (this.reader.eof()) {
            this.halted = true;
            if (this.verbose) {
                console.log(`${this.name}: program out of bounds`);
            }
            return false;
        }
